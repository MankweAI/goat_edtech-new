 ...
 * GOAT Bot 2.0
 * Updated: 2025-08-28 13:20:00 UTC
 * Developer: DithetoMokgabudi
 *
 * Flow:
 * 1) Ask for subject and grade (single turn): "Mathematics, 10"
 * 2) List topics for subject -> user picks
 * 3) List sub-topics (derived from examples) -> user picks
 * 4) Generate first question; difficulty progresses automatically based on interactions (hints/solutions/next)
 */
...
function parseSubjectGrade(text) {
  ...
  // Find a grade number anywhere
  const gradeMatch = raw.match(/\b(gr(?:ade)?\s*)?(\d{1,2})\b/i);
  const grade = gradeMatch
    ? Math.max(8, Math.min(12, parseInt(gradeMatch[2], 10)))
    : 10;

  return { subject, grade };
}

function listTopicsForSubject(subject) {
  const bank = SUBJECT_PROBING_DATABASE[subject] || {};
  // Filter only direct keys with objects
  const topics = Object.keys(bank).filter((k) => typeof bank[k] === "object");
  return topics;
}

function deriveSubtopics(subject, topicKey) {
  const bank = SUBJECT_PROBING_DATABASE[subject] || {};
  const topic = bank[topicKey] || {};
  // Use 'examples' as sub-topics; fallback to common_struggles; ensure unique and non-empty
...
// Progressive difficulty controller
function getDifficultyByProgression(level = 0) {
  // 0 -> simplified, 1 -> mixed, 2+ -> challenging
...
function updateProgressionOnAction(qm, action) {
  ...
  if (action === "hint") {
    ...
    qm.stats.nexts += 1;
    // Reward: if no hint/solution in last question, increase
    const recentHelp = (qm.lastQuestion || {}).helpUsed || false;
    if (!recentHelp) qm.progression += 1;
    // Cap progression to 3 for stability
...
async function startAIQuestionsMode(user) {
  ...
  user.context.questions_mode = {
    progression: 0, // start simple and ramp up
    q_index: 0,
...
async function handleTopicSelect(user, text) {
  ...
  qm.topic = chosenKey.charAt(0).toUpperCase() + chosenKey.slice(1);

  const subs = deriveSubtopics(qm.subject || "Mathematics", chosenKey);
  if (!subs || subs.length === 0) {
    // If no subtopics, jump straight to question loop with topic as subtopic
...
Just pick a number! ✨`;
  ...
  if (wantsChangeTopic(t)) {
    // Reset to topic selection with current subject
    user.context.ai_intel_state = AI_INTEL_STATES.TOPIC_SELECT;
    const topics = listTopicsForSubject(qm.subject || "Mathematics");
    user.context.questions_mode._topics = topics;
    const list = formatNumberedList(
      topics.map((x) => x.charAt(0).toUpperCase() + x.slice(1))
    );
    const content = `Okay, pick a topic:\n\n${list}`;
    return formatResponseWithEnhancedSeparation(
      content,
      `Reply with a number (e.g., 1)`,
      user.preferences.device_type
  ...
  if (wantsChangeSubtopic(t)) {
    // Reset to subtopic selection for the current topic key (find key by lowercase match)
    const subject = qm.subject || "Mathematics";
    const topicKey = (qm.topic || "").toLowerCase();
    const topics = listTopicsForSubject(subject);
    const key = topics.find((k) => k.toLowerCase() === topicKey) || topics[0];
    const subs = deriveSubtopics(subject, key);
    user.context.ai_intel_state = AI_INTEL_STATES.SUBTOPIC_SELECT;
...
module.exports = {
  ...
  // Back-compat aliases (unused in this flow)
