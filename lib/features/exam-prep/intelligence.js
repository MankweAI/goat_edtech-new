 ...
 * GOAT Bot 2.0
 * Updated: 2025-08-28 14:20:00 UTC
 * Developer: DithetoMokgabudi
 *
 * Flow:
 * 1) Ask for subject and grade (single turn): "Mathematics 10"
 * 2) List CAPS topics for the subject+grade -> user picks
 * 3) List CAPS sub-topics -> user picks
 * 4) Unlimited practice questions; difficulty progresses automatically
 * 5) User can force difficulty up/down and switch topic anytime
 */
...
function parseSubjectGrade(text) {
  ...
  // Accept "Mathematics 10", "Maths 11", "Physical Sciences: 10"
  const parts = raw
    .split(/[,\|;:-]/)
    .map((s) => s.trim())
    .filter(Boolean);
...
function listTopicsForSubject(subject, grade) {
  ...
  // Fallback: legacy DB (subject-only)
...
function updateProgressionOnAction(qm, action) {
  // action: "next" | "hint" | "solution" | "harder" | "easier"
...
async function startAIQuestionsMode(user) {
  ...
  user.context.questions_mode = {
    progression: 0, // start at Foundation
    q_index: 0,
...
async function handleTopicSelect(user, text) {
  ...
  if (!subs || subs.length === 0) {
    // No subtopics; jump straight into questions
...
// Keep sub-topic changer available by phrase only (no numeric conflict with Harder/Easier)
