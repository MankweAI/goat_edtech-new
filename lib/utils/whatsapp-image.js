 ...
 *
 * Changes:
 * - Add media circuit breaker to avoid blocking user flows on repeated WA API failures (404/timeouts).
 * - Enforce total send budget across endpoints; fail fast when budget is exhausted.
 * - Reduce per-endpoint timeout; prefer quick fallthrough.
 * - Keep resilient endpoint fallback order and SVG→PNG conversion.
 */
...
function recordMediaFailure(reason = "") {
  ...
  if (MEDIA_CB.failures >= MEDIA_CB.threshold) {
    ...
    // Reset failures while open; we'll close by time
    MEDIA_CB.failures = 0;
  }
}
...
async function postToManyChatEndpoints(
  ...
  // Per-attempt timeout (cap) with env override; adjust on remaining budget
  const defaultAttemptMs = parseInt(
    process.env.MANYCHAT_MEDIA_ATTEMPT_TIMEOUT_MS || "7000",
    10
  );
  ...
  for (const url of endpoints) {
    ...
    try {
      ...
      // Count 5xx/timeouts as breaker failures; 404 indicates invalid path (we'll try next)
      if (status >= 500) recordMediaFailure(`HTTP_${status}`);
      lastErr = new Error(`Endpoint failed (${url}): HTTP ${status}`);
      continue;
    } catch (e) {
      // network/timeout → breaker input
