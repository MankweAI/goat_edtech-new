 ...
 * GOAT Bot 2.0
 * Updated: 2025-08-29 10:33:00 UTC
 * Developer: DithetoMokgabudi
 *
 * Fix:
 * - Replace problematic character-class escapes causing "Invalid regular expression" on Node 18.
 * - Safer unicode-friendly whitelist without unnecessary escapes.
 * - Keep behavior: do NOT render when expression is readable with plain text + unicode.
 */
...
function isSimpleUnicodeMath(text) {
  ...
  // If any LaTeX-ish backslashes exist, it's not "simple unicode"
  if (/[\\]/.test(t)) return false;

  // Allowed characters: letters, digits, whitespace, core operators, comparisons, punctuation,
  // common unicode math glyphs. Avoid unnecessary escapes that break older engines.
  const allowedPattern = /^[\s0-9A-Za-z+\-*×÷=<>≤≥≠()\[\]{}.,:;\/|%^√πθ∞±∓]+$/u;

  return allowedPattern.test(t);
}
